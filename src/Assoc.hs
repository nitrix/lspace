...
lookup :: (Ord a, Hashable a) => a -> Assoc a b -> S.Set b
lookup k (MkAssoc left _) = fromMaybe S.empty $ M.lookup k left

lookupR :: (Ord b, Hashable b) => b -> Assoc a b -> S.Set a
lookupR k (MkAssoc _ right) = fromMaybe S.empty $ M.lookup k right

fromList :: (Ord a, Ord b, Hashable a, Hashable b) => [(a, b)] -> Assoc a b
fromList xs = MkAssoc
...
adjust :: (Ord a, Ord b, Hashable a, Hashable b) => (b -> b) -> a -> Assoc a b -> Assoc a b
...
adjustR :: (Ord a, Ord b, Hashable a, Hashable b) => (a -> a) -> b -> Assoc a b -> Assoc a b
...
insert :: (Ord a, Ord b, Hashable a, Hashable b) => a -> b -> Assoc a b -> Assoc a b
